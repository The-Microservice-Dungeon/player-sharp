...
public class TradeEvent<T> where T : class
{
    ...
    [JsonPropertyName("moneyChangeBy")] public int MoneyChangeBy { get; set; }
...
public class TradingTradesBuyRobotEventConsumer : DungeonEventConsumer<string, TradeEvent<BuyRobotData>>
{
    ...
    {
        var type = Encoding.UTF8.GetString(cr.Message.Headers.Where(header => header.Key == "type").First()
            .GetValueBytes());
        var transactionId = Encoding.UTF8.GetString(cr.Message.Headers.Where(header => header.Key == "transactionId")
            .First().GetValueBytes());
