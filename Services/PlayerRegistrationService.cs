...
namespace Player.Sharp.Services
{
    public class PlayerRegistrationService : IHostedService
    {
        ...
        public PlayerRegistrationService(IGameClient gameClient, ILogger<PlayerRegistrationService> logger, IPlayerCredentialsRepository playerCredentialsRepository)
        {
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested && !_playerCredentialsRepository.Exists())
            {
                try
                {
                    var response = await _gameClient.GetPlayerDetails(PlayerConstants.PLAYER_NAME, PlayerConstants.PLAYER_EMAIL);
